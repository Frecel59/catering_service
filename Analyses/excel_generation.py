...
def generate_excel_report(result_df, start_date, end_date):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        header_format = writer.book.add_format({
            'bold': True,         # En gras
            'text_wrap': True,    # Saut de ligne automatique pour les en-têtes longs
        ...
        # Appliquer le format correspondant
        worksheet.set_column('B:B', len('Nbr Couverts') + 2, custom_number_format_B)

        # Créez un format personnalisé pour la colonne C (%)
        custom_percent_format_C = writer.book.add_format({
            'num_format': '0.00%',  # Format pourcentage avec 2 décimales
            'font_size': 11  # Taille de police
        })
        ...
        custom_compte_format_D = writer.book.add_format({
            'num_format': '#,##0.00 €',  # Format comptabilité avec 2 décimales et le symbole €
            'font_size': 11  # Taille de police
        })

        # Appliquer le format correspondant
        worksheet.set_column('D:D', len('Total Additions €') + 2, custom_compte_format_D)
        ...
        # Appliquer le format correspondant
        worksheet.set_column('E:E', len('Panier moyen €') + 2, custom_compte_format_E)
