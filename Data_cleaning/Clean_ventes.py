...
def clean_file_ventes(excel_file_stream):
...
def clean_file_from_gcs_ventes(blob):
    """Lire le fichier excel et retourner un dataframe clean."""
    in_mem_file = BytesIO()
    blob.download_to_file(in_mem_file)
    df = clean_file_ventes(in_mem_file)
    in_mem_file.close()

    return df


def clean_files_in_bucket_ventes():
    all_dataframes = []
...
if __name__ == '__main__':
    print(clean_files_in_bucket_ventes().Date.min())
